...
# -*- coding: utf-8 -*-
"""
Created on Fri May 12 17:20:42 2023
...
raw_data = scipy.io.loadmat('banana.mat')

##Training data (e.g, wbcd dataset)
# ===========================================
#  Wisconsin breast cancer data
#  source: https://www.kaggle.com/uciml/breast-cancer-wisconsin-data
data = raw_data['banana']
nData = len(data)
#print(data.type)
...
h=1e-4 # time step
numSteps=int(3.5e3+1) #1e6 # number of steps k, in discretization t=kh

a_min, a_max=0.9, 1.1; # min-max scaling
b_min, b_max=-0.3, 0.3; # min-max bias
w_min, w_max=-2, 2;
numRandomRun=5 # 50
lambda_1=torch.rand((nSample,1), device=device, dtype=dtype)
...
#acc_test = torch.zeros((numRandomRun, 500000), device=device, dtype=dtype)
#acc_test_weight = torch.zeros((numRandomRun, 500000), device=device, dtype=dtype)
for r in range(numRandomRun):
    ...
    rho_prev=torch.ones((nSample,1), device=device, dtype=dtype)*1/1000#(1/((a_max-a_min)*(b_max-b_min)*((w_max-w_min)**nx)))
    ...
    for k in range(numSteps):
        ...
        if (k%3.5e3 == 0):
            ...
            for i in range(nData_test):
                ...
                if f_hat_test[i] < 0:
                    f_hat_test[i]=0
            acc_test_weight = sum(f_hat_weight_test==labels_test)/nData_test
            print(acc_test_weight)
            acc = sum(f_hat_weight==labels)
            print(acc/nData)
            acc_test = sum(f_hat_test==labels_test)/nData_test
            print(acc_test)
